    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {/* Blank Event Option */}
          <Card className="border-dashed hover:border-primary/50 transition-colors cursor-pointer" onClick={() => handleSelectTemplate(null as any)}>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">Start from scratch with no preset values</p>
              </div>
          ...
          {isLoading ? (
                  ...
                  <CardContent className="pt-0">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {template.guest_limit && (
                        <Badge variant="secondary" className="gap-1">
                          <span className="text-xs">👥</span> {template.guest_limit} guests
                        </Badge>
                      )}
                    </div>
                  </CardContent>
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              {search && <p className="text-sm text-muted-foreground mt-1">Try a different search</p>}
            </div>
          )}
        </div>
      </DialogContent>
