...
import defaultBanner from "@/assets/default-event-banner.jpeg";
...
const PublicEvent = () => {
    ...
    const fetchSeats = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/public-registrations?event_id=${event.id}&count_only=true`,
          {
  ...
  useEffect(() => {
    if (!event) return;
    document.title = `${event.title} | Regixo`;
    ...
    setMeta(
      "og:description",
      event.venue ? `${event.venue} — Register now!` : "Register now!",
    );
    setMeta("og:type", "website");
    setMeta("og:url", window.location.href);
    if (event.banner_url) setMeta("og:image", event.banner_url);
    return () => {
      document.title = "Regixo";
    };
  }, [event]);
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        {event.description && (
          <div className="rounded-md bg-amber-400/20 shadow-sm border border-gray-300 p-5 sm:p-6 space-y-4">
            ...
            <div
              className="prose prose-sm max-w-none text-muted-foreground [&_h1]:text-foreground [&_h2]:text-foreground [&_h3]:text-foreground [&_strong]:text-foreground [&_a]:text-primary [&_a]:no-underline [&_a:hover]:underline"
              style={{
        ...
        {event.instructions && (
          <div className="rounded-md bg-green-300/20 shadow-sm border border-gray-300 overflow-hidden">
            ...
            <div className="p-5 sm:p-6 space-y-4">
              ...
              <div
                className="prose prose-sm max-w-none text-muted-foreground [&_h1]:text-foreground [&_h2]:text-foreground [&_h3]:text-foreground [&_strong]:text-foreground [&_a]:text-primary"
                style={{
...
// payment_number JSON parse — {"bKash":"017...","Nagad":"018..."} বা plain string
    ...
    try {
      ...
      if (typeof parsed === "object" && parsed !== null) {
        const methods = paymentMethods?.length
          ...
          : Object.keys(parsed);
        return methods
          .filter((m: string) => parsed[m])
          .map((m: string) => ({ method: m, number: parsed[m] }));
      }
    } catch {}
  }
  // Legacy plain string
  const firstMethod = paymentMethods?.[0] || "Payment";
  return [{ method: firstMethod, number: raw }];
}
...
const MetaCard = ({
  ...
  <div
    className={`flex items-start gap-3 p-3.5 rounded-md border transition-colors ${
      danger
...
const SuccessView = ({
  ...
  <div className="py-2 space-y-5">
    ...
    {trxWarning && (
      <div className="flex items-start gap-2.5 rounded-2xl border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-900/40 p-4">
...
const RegistrationForm = ({
  ...
  // Parse করো — 1টা বা একাধিক payment entry
    ...
    <form className="space-y-4 pt-1" onSubmit={handleSubmit}>
      ...
      {hasPaidPayment && (
        <div className="rounded-md bg-primary/5 border border-primary/20 p-4 space-y-3">
          ...
          {/* প্রতিটা method এর জন্য আলাদা row + copy button */}
          <div className="space-y-2.5">
              ...
              <div key={method}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="h-10 w-10 flex items-center justify-center rounded-md border border-border/50 bg-green-100 dark:bg-green-900/30 hover:bg-green-200 dark:hover:bg-green-800/40 transition-colors shrink-0"
                    title={`Copy ${method} number`}
                  >
          ...
          {/* একাধিক method থাকলে "যেকোনো একটায় পাঠান" hint */}
        ...
        <div key={field.id} className="space-y-1.5">
          ...
          {field.field_type === "checkbox" && (
            <div className="space-y-2">
              {(field.field_options as string[])?.filter((o: string) =>
                o.trim(),
              ).length > 0 ? (
                (field.field_options as string[])
                  .filter((o: string) => o.trim())
...
const RegisteredListModal = ({
  ...
  const fetchList = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/public-registrations?event_id=${eventId}`,
        { headers: { apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY } },
      );
      const json = await res.json();
