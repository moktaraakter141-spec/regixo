...
import CustomFieldsBuilder, {
  type CustomField,
} from "@/components/CustomFieldsBuilder";
...
// ── CHANGE 1: FormState এ show_default_banner যোগ করা হয়েছে ──
interface FormState {
  title: string;
  description: string;
...
// ── CHANGE 2: initialForm এ show_default_banner: false যোগ করা হয়েছে ──
...
// ── CHANGE 3: buildEventData এ show_default_banner যোগ করা হয়েছে ──
...
const EventForm = () => {
  ...
  // ── CHANGE 4: Edit mode init এ show_default_banner যোগ করা হয়েছে ──
  useEffect(() => {
    if (!isEdit) return;
    if (!dataReady && isEdit) return;
  ...
  const autoSave = useCallback(async () => {
    ...
    try {
      ...
      const eventData = buildEventData(
        currentForm,
        user.id,
        currentEvent?.slug || null,
        (currentEvent?.status as any) || "draft",
      );
  ...
  const addPaymentMethod = (method: string) => {
    ...
    const newEntries = [...form.payment_method_entries, { method, number: "" }];
    update("payment_method_entries", newEntries);
  };
  ...
  const removePaymentMethod = (method: string) => {
    ...
    const newInstruction = generateInstruction(newEntries, form.price);
    setForm((prev) => ({
      ...prev,
      payment_method_entries: newEntries,
      payment_instruction: newInstruction,
    }));
    setTouched((prev) => {
      const next = new Set(prev);
      next.add("payment_method_entries");
      next.add("payment_instruction");
      return next;
    });
    if (isEdit) {
      if (autoSaveTimeout.current) clearTimeout(autoSaveTimeout.current);
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                />
                {/* ── CHANGE 5: Default banner toggle ── */}
                {!form.banner_url && (
