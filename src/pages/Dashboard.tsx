...
const Dashboard = () => {
  ...
  // Fetch registration counts per event for seat tracking
  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      <div className="flex-1 min-w-0">
        <header className="sticky top-0 z-30 flex items-center h-16 px-4 sm:px-6 border-b bg-background/80 backdrop-blur-sm">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            >
              {dark ? (
                <Sun className="h-4 w-4 items-center " />
              ) : (
              ...
              <div className="border bg-card overflow-hidden rounded-lg">
                ...
                {filteredEvents.map((event, i) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className={`group flex sm:grid sm:grid-cols-[1fr_100px_120px_100px_44px] items-center gap-3 px-5 py-3.5 hover:bg-muted/30 transition-colors cursor-pointer
        ${i < filteredEvents.length - 1 ? "border-b" : ""}`}
                      onClick={() =>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-8 w-8 text-muted-foreground opacity-100 transition-opacity"
                            >
      ...
      <TemplateSelector
        ...
        onSelect={(template) => {
          if (template) {
            navigate(`/events/new?template=${template.id}`);
          } else {
            navigate("/events/new");
          }
        }}
